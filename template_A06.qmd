...
[https://github.com/SSajjad1/Sajjad_Sarah_201_A06](https://github.com/SSajjad1/Sajjad_Sarah_201_A06)
...
# iterate through each player and calculate points per game
for(i in names(player_info)){
  player <- player_info[[i]] 
  points_per_game[[i]] <- player$points / player$games_played
}
...
```{r}
# sapply outputs a vector
rounded_points <- sapply(points_per_game, function(x) round(x, 2))
...
```{r}
# User defined function rounds values by number of digits
round_func <- function(x){
 ...
 # apply function using defined function
# MARGIN = 1:2 applyes function all each value
mat_rounded <- apply(mat_rand, MARGIN = 1:2, round_func)
...
```{r}
# column means, for columns 3 to 14 (each month)
avg_temp <- colMeans(max_temps[ ,3:14])
avg_temp
```
...
co_data <- max_temps[1:126, ]

oh_data <- max_temps[127:252, ]

va_data <- max_temps[253:378, ]
...
```{r}
med_temp <- sapply(co_data, median, na.rm = TRUE)
med_temp

which.max(med_temp)
which.min(med_temp)

paste0("The maximum median annual temperature of ", max(med_temp), " occurred in ", which.max(med_temp)," and the minimum median annual temperature of ", min(med_temp), " occurred in ", which.min(med_temp), ".")
```
...
if (n == 0){ # base case
  ...
  last_digit <- n %% 10  # takes remainder
  trunc_num <- n %/% 10  #ignores remainder
  return(last_digit + digits_sum(trunc_num))
  }
}

# run function
run_func <- digits_sum(935068)
run_func
```
